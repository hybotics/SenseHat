...
'''
  ...
  TODO:     Have script check for *changes* in the confuration file and reread settings if changes were made
'''
...
'''
  Possible additions to the configuration file
'''
...
MAX_LOOPS = 10
'''
  End of possible new configuration settings
'''
# Set up colors
RED			      =	(255, 0, 0)
GREEN		      =	(0, 255, 0)
...
def find_setting(setting, settings_list):
  ...
  if DEBUGGING:
    for n in range(slen):
      print(f"(find_setting) {n + 1} : {settings_list[n]}")
...
def get_mod_time(cf_name):
  '''
    Return the modification time of a file
  '''
  if path.exists(cf_name):
    fstat = stat(cf_name, follow_symlinks=False)
    result = fstat.st_mtime
  else:
    result = False
...
def get_settings(cf_name, s_labels, s_types):
  '''
    Get settings from a key=value pair style configuration file
  '''
  settings_list = []
  result = []
  ...
  if path.exists(cf_name):
    ...
    if DEBUGGING:
      print(f"(get_settings) Settings List = {settings_list}")
...
def init_config_file(cf_name=CONFIG_FILE_NAME):
  '''
...
settings_list = get_settings(CONFIG_FILE_NAME, SETTINGS_LABELS, SETTINGS_TYPES)

if settings_list is []:
  print(f"Configuration file {CONFIG_FILE_NAME} does not exist - initializing!")
  settings_list = init_config_file(CONFIG_FILE_NAME)

DEBUGGING = find_setting("debugging", settings_list)

if DEBUGGING:
  if settings_list != []:
    slen = len(settings_list)
    print(f"There are {slen} settings in the file.")

    for setting_nr in range(slen):
      print(f"Setting: {settings_list[setting_nr][0]}, Value = {settings_list[setting_nr][1]}")
  else:
    emsg = "There are no settings in the file!"
    raise ValueError(emsg)

#change_settings(settings_list)
...
curr_modification_time = get_mod_time(CONFIG_FILE_NAME)

if not curr_modification_time:
  print()
  print(f"Configuration file {CONFIG_FILE_NAME} does not exist - initializing!")
  init_config_file(CONFIG_FILE_NAME)
  curr_modification_time = get_mod_time(CONFIG_FILE_NAME)
...
try:
  while True:
    ...
    # Update the configurations settings if the file has changed
    if last_modification_time < curr_modification_time or curr_modification_time:
      last_modification_time = curr_modification_time
      settings_list = get_settings(CONFIG_FILE_NAME, SETTINGS_LABELS, SETTINGS_TYPES)

      USING_DATA_CONSOLE = find_setting("data_console", settings_list)
      USING_TEXT_CONSOLE = find_setting("text_console", settings_list)
      DEFAULT_READING_INTERVAL_MIN = find_setting("interval", settings_list)
      USING_SENSE_RGB_MATRIX = find_setting("use_matrix", settings_list)
      USING_DATA_LOGGING = find_setting("data_logging", settings_list)
